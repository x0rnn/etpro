...
m = [0, 1, 2, 4, 8, 16, 32 , 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216, 33554432, 67108864, 134217728]
...
for filename in logs:
	...
	for line in open(r'' + filename + '', encoding="ISO-8859-1"):
		...
		if prev_line:
			...
			match_id = re.search(r'ClientConnect:\s*(\d{1,2})\n.*ClientUserinfoChanged:\s*\d{1,2}\s*n\\(.+?)\\', comb_lines)
			if match_id:
				name = match_id.group(2)
				name = re.sub(r'\^\^', '©', name)
				name = re.sub(r'\^.', '', name)
				name = re.sub(r'©', '^', name)
				players[int(match_id.group(1))] = name
				...
				if players[id] is not None:
					...
					if mask not in m:
						...
						for j in range(bits_len):
							if bits[j] == 1 or bits[j] == 2 or bits[j] == 4 or bits[j] == 8 or bits[j] == 16 or bits[j] == 32 or bits[j] == 64:
								if bits[j] == 1 and (j == bits_len - 1 or bits[j+1] != 2):
									knife = True
								elif bits[j] == 2 and (j == 0 or bits[j-1] != 1):
									knife = True
								elif bits[j] == 1 and bits[j+1] == 2:
									both_knives = True
								else:
									w += 1
						if w != 0:
							if knife == True:
								...
								if w == 1:
									...
									match_wm = re.search(r'\d*\s*\d*\s*\d*\s*\d*\s*\d*\s*\d*\s*\d*\s*\d*\s*\d*\s*\d*\s*(\d*)\s*(\d*)\s*(\d*)\s*\d*\s*(\d*)\s*(\d*)\s*(\d*)\s*(\d*)\s*\d*\s*(\d*)\s*(\d*)\s*(\d*)\s*(\d*)\s*\d*\s*(\d*)\s*(\d*)\s*(\d*)\s*(\d*)\s*\d*\s*(\d*)\s*(\d*)\s*(\d*)\s*(\d*)\s*\d*\s*(\d*)\s*.*', match_ws.group(3))
									hits1 = int(match_wm.group(1))
									shots1 = int(match_wm.group(2))
									kills1 = int(match_wm.group(3))
									hs1 = int(match_wm.group(4))
						...
						if mask == 4 or mask == 8 or mask == 16 or mask == 32 or mask == 64:
							match_wm = re.search(r'(\d*)\s*(\d*)\s*(\d*)\s*\d*\s*(\d*)\s*.*', match_ws.group(3))
							hits = int(match_wm.group(1))
							shots = int(match_wm.group(2))
							kills = int(match_wm.group(3))
